        ...
        if (empty($title)) {
            ...
            try {
                ...
                if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                    $upload_dir = SITE_ROOT . 'assets/images/services/';
        ...
        if ($service_id > 0) {
            try {
                ...
                if ($service && !empty($service['image'])) {
                    $image_file = SITE_ROOT . $service['image'];
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="title" name="title" value="<?php echo $service ? $service['title'] : ''; ?>" required>
                        </div>
                        
                        <div class="mb-3">
                            <label for="description" class="form-label">Description <span class="text-danger">*</span></label>
                            <textarea class="form-control" id="description" name="description" rows="5" required><?php echo $service ? $service['description'] : ''; ?></textarea>
                        </div>
                        
                        <div class="mb-3">
                            <label for="icon" class="form-label">Icon Class <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <span class="input-group-text"><i id="icon-preview" class="<?php echo $service ? $service['icon'] : 'fas fa-cog'; ?>"></i></span>
                                <input type="text" class="form-control" id="icon" name="icon" value="<?php echo $service ? $service['icon'] : ''; ?>" placeholder="e.g., fas fa-chalkboard-teacher" required>
                            </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <?php if ($service && !empty($service['image'])): ?>
                                <div class="mt-2">
                                    <p>Current Image:</p>
                                    <img src="<?php echo SITE_URL . '/' . $service['image']; ?>" alt="Current Image" class="img-thumbnail" style="max-height: 150px;">
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="display_order" name="display_order" value="<?php echo $service ? $service['display_order'] : '0'; ?>" min="0">
                ...
                <div class="mt-4">
                    <button type="submit" name="<?php echo $action === 'add' ? 'add_service' : 'edit_service'; ?>" class="btn btn-primary">
                        ...
                        <tbody>
                            <?php foreach ($services as $service): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?php echo ADMIN_URL; ?>/pages/services.php?action=edit&id=<?php echo $service['id']; ?>" class="btn btn-sm btn-warning">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="" method="post" class="d-inline">
                                            <input type="hidden" name="service_id" value="<?php echo $service['id']; ?>">
                                            <button type="submit" name="delete_service" class="btn btn-sm btn-danger confirm-delete" data-bs-toggle="tooltip" title="Delete">
